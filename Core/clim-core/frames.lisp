...
(defmethod frame-all-layouts ((frame application-frame))
  (map 'list #'car (frame-layouts frame)))
...
(defun update-frame-pane-lists (frame)
  (let ((all-panes     (frame-panes frame))
    ...
    (map-over-sheets (lambda (sheet)
                     ...
                     all-panes)
    (setf current-panes (map 'list #'car (sort current-panes #'< :key #'cdr)))
    ...
    (let ((interactor (find-pane-of-type current-panes 'interactor-pane)))
      (setf (frame-current-panes frame) current-panes
            ...
            (frame-standard-output frame)
            (or (find-pane-of-type current-panes 'application-pane) interactor)

            (frame-standard-input frame)
            (or interactor (frame-standard-output frame))

            (frame-pointer-documentation-output frame)
            (find-pane-of-type all-panes 'pointer-documentation-pane)))))
